...
\anoop{Koehn has a good example of alignment and phrase extraction in his tutorial slides. You can copy the figures from there and don't forget to cite the tutorial. see page 13-14 and page 26 in: http://homepages.inf.ed.ac.uk/pkoehn/publications/tutorial2003.pdf}
...
\anoop{make sure that you do not stick parens with the previous word as in the following paragraph.}
...
\anoop{use consistent capital letter for Table X}
...
\anoop{explain the log linear model for SMT here. put the equation of the log-linear model here as well.}
...
%Hence, if a longer source phrase aligns to an equally long target phrase, it can be penalized if the individual words are not aligned. 

\anoop{now explain how to learn the weights of the log-linear model introduced above}
...
\anoop{do not use abbreviations especially if you have not introduced the full length version before}
...
\anoop{what is ``error count'' metric mean? do you just mean the evaluation metric? what is $C$? what is $E(\cdot)$ in equation 1.6 below?}
...
\anoop{this eqn needs some parentheses. what is $d$ below? the eqn looks incorrect to me.}
...
\anoop{chapter ends very abruptly. can you introduce what happens next?}
